...
if (winWidth <= 992) {
  images = [{ id: 'light', src: bglight }, { id: 'dark', src: bgdark }];
}
if (winWidth > 992) {
  images = [
...
Promise.all([preloadImage(images[0]), preloadImage(images[1])])
  .then(images => {
    ...
    lightImage.setAttribute(
      'style',
      'background-image:url(' + byId['light'] + ');'
    );
    darkImage.setAttribute(
      'style',
      'background-image:url(' + byId['dark'] + ');'
    );
